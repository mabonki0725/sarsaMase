...
int main(int argc,char *argv[]) 
{
  ...
  if(argc >= 3) {
    if(!(fw=fopen(argv[2],"w"))) {
      fprintf(stderr,"cannot write outputFile=[%s]\n",argv[2]);
      exit(-2);
    }
  }
  g_fdbg=NULL;
  if(argc >= 4) {
    if(!(g_fdbg=fopen(argv[3],"w"))) {
      fprintf(stderr,"cannot write dbgFile=[%s]\n",argv[3]);
      exit(-2);
    }
  }
  /* input data size */
  ...
  for(run=0; run<2; run++) {
    ...
    for(episode_num=0; episode_num<100; episode_num++) {
      ...
      if(g_fdbg) fprintf(g_fdbg,"round,%d execute episode(10000),%d\n",episode_num,ret);
    }
  }
